...
if __name__ == '__main__':
    ...
    socket_in_config = extract_config(conf, 'local_server_xpub',
                                     'socket_in_')

    socket_in1_config = extract_config(conf, 'local_server_xpub',
                                      'socket_in1_')

    socket_out_config = extract_config(conf, 'local_server_xpub',
                                     'socket_out_')

    mirror_socket_config = extract_config(conf, 'local_server_xpub',
                                          'mirror_socket_')
    ...
    if socket_in1_config is not None:
        if '://' not in socket_in1_config['url']:
            socket_in1_config['url'] = os.environ[socket_in1_config['url']]

        temp = socket_in_config['url']

        socket_in_config['url'] = [temp, socket_in1_config['url']]

    pprint({'socket_in_config': socket_in_config})
